...
#define USE_TF                                                                 \
  true /* leave true, except when synthesizing some specific instructions */
...
ignore_op_t opcode_blacklist[MAX_BLACKLIST] = {
    ...
    {"\xbc", "mov sp"},
    /* segfaulting with "shr sp, 1" (always sp) in random synthesizing, too
     * tired to figure out why: 66 d1ec */
...
range_t total_range = {
    .start = {.bytes = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                        0x00, 0x00, 0x00, 0x00, 0x00, 0x00},
              .len = 0},
    .end = {.bytes = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
...
void initialize_ranges(void) {
  if (range_marker == NULL) {
    range_marker = mmap(NULL, sizeof *range_marker, PROT_READ | PROT_WRITE,
...
bool move_next_range(void) {
  ...
  case TUNNEL:
    ...
    if (memcmp(range_marker->bytes, total_range.end.bytes,
      ...
      // #error make sure you don't skip over the first instruction (e.g.
      // 000000...) #error there's another error here somewhere... somehow take
      // start range from end range.. len can mostly be taken from range_bytes
      // WHEN YOU MOVE TO A NEW RANGE but needs to be from
      // total_range.start/end.len when you are deriving from that right now len
      // is set in total_range, and in increment_range for range.end
      ...
      } else if (memcmp(search_range.end.bytes, total_range.end.bytes,
                        sizeof(search_range.end.bytes)) > 0) {
...
int print_asm(FILE *f) {
  if (output == TEXT) {
    ...
    if (cs_disasm_iter(capstone_handle, (const uint8_t **)&code, &code_size,
                       &address, capstone_insn)) {
      sync_fprintf(f, "%10s %-45s (%2d)", capstone_insn[0].mnemonic,
                   capstone_insn[0].op_str,
                   (int)(address - (uintptr_t)packet_buffer));
    } else {
      sync_fprintf(f, "%10s %-45s (%2d)", "(unk)", " ",
                   (int)(address - (uintptr_t)packet_buffer));
    }
...
void print_mc(FILE *f, int length) {
  ...
  for (i = 0; i < length && i < MAX_INSN_LENGTH; i++) {
    ...
    if (!p && i < MAX_INSN_LENGTH - 1 && is_prefix(inj.i.bytes[i]) &&
...
void inject(int insn_size) {
  /* could probably fork here to avoid risk of destroying the controlling
   * process */
  ...
  __asm__ __volatile__(
      "\
			...
			"
      : /* no output */
      : [rax] "m"(inject_state.rax), [rbx] "m"(inject_state.rbx),
        [rcx] "m"(inject_state.rcx), [rdx] "m"(inject_state.rdx),
        [rsi] "m"(inject_state.rsi), [rdi] "m"(inject_state.rdi),
        [r8] "m"(inject_state.r8), [r9] "m"(inject_state.r9),
        [r10] "m"(inject_state.r10), [r11] "m"(inject_state.r11),
        [r12] "m"(inject_state.r12), [r13] "m"(inject_state.r13),
        [r14] "m"(inject_state.r14), [r15] "m"(inject_state.r15),
        [rbp] "m"(inject_state.rbp), [rsp] "i"(&dummy_stack.dummy_stack_lo),
        [packet] "m"(packet));
#else
  __asm__ __volatile__(
      "\
			...
			"
      :
      : [eax] "m"(inject_state.eax), [ebx] "m"(inject_state.ebx),
        [ecx] "m"(inject_state.ecx), [edx] "m"(inject_state.edx),
        [esi] "m"(inject_state.esi), [edi] "m"(inject_state.edi),
        [ebp] "m"(inject_state.ebp), [esp] "i"(&dummy_stack.dummy_stack_lo),
        [packet] "m"(packet));
#endif
...
void fault_handler(int signum, siginfo_t *si, void *p) {
   ...
   */
  insn_length = (uintptr_t)uc->uc_mcontext.gregs[IP] - (uintptr_t)packet -
                preamble_length;
  ...
  result = (result_t){1, insn_length, signum, si->si_code,
                      (signum == SIGSEGV || signum == SIGBUS)
                          ? (uint32_t)(uintptr_t)si->si_addr
                          : (uint32_t)-1};

  memcpy(uc->uc_mcontext.gregs, fault_context.gregs,
         sizeof(fault_context.gregs));
...
void get_rand_insn_in_range(range_t *r) {
  ...
  for (i = 0; i < MAX_INSN_LENGTH; i++) {
    if (all_max && all_min) {
      inj.i.bytes[i] = rand() % (inclusive_end[i] - r->start.bytes[i] + 1) +
                       r->start.bytes[i];
    } else if (all_max) {
  ...
  case TEXT:
    ...
    case DRIVEN:
      ...
      if (result.signum == SIGILL) {
        sync_fprintf(f, "sigill ");
      }
      if (result.signum == SIGSEGV) {
        sync_fprintf(f, "sigsegv");
      }
      if (result.signum == SIGFPE) {
        sync_fprintf(f, "sigfpe ");
      }
      if (result.signum == SIGBUS) {
        sync_fprintf(f, "sigbus ");
      }
      if (result.signum == SIGTRAP) {
        sync_fprintf(f, "sigtrap");
      }
...
void help(void) {
  ...
  printf("\t[-b|-r|-t|-d] ....... mode: brute, random, tunnel, directed "
         "(default: tunnel)\n");
  printf("\t[-R|-T] ............. output: raw, text (default: text)\n");
  printf("\t[-x] ................ show tick (default: %d)\n", config.show_tick);
  printf("\t[-0] ................ allow null dereference (requires sudo) "
         "(default: %d)\n",
  ...
  printf("\t[-B brute_depth] .... in brute search, maximum search depth "
         "(default: %d)\n",
         config.brute_depth);
  printf("\t[-P max_prefix] ..... maximum number of prefixes to search "
         "(default: %d)\n",
         config.max_prefix);
  printf("\t[-i instruction] .... instruction at which to start search, "
         "inclusive (default: 0)\n");
  printf("\t[-e instruction] .... instruction at which to end search, "
         "exclusive (default: ff..ff)\n");
  printf("\t[-c core] ........... core on which to perform search (default: "
         "any)\n");
  printf("\t[-X blacklist] ...... blacklist the specified instruction\n");
  printf("\t[-j jobs] ........... number of simultaneous jobs to run (default: "
         "%d)\n",
         config.jobs);
  printf("\t[-l range_bytes] .... number of base instruction bytes in each sub "
         "range (default: %d)\n",
         config.range_bytes);
}
...
int main(int argc, char **argv) {
  ...
  pool_mutex = mmap(NULL, sizeof *pool_mutex, PROT_READ | PROT_WRITE,
                    MAP_SHARED | MAP_ANONYMOUS, -1, 0);
  output_mutex = mmap(NULL, sizeof *output_mutex, PROT_READ | PROT_WRITE,
                      MAP_SHARED | MAP_ANONYMOUS, -1, 0);
  ...
  packet_buffer =
      (void *)(((uintptr_t)packet_buffer_unaligned + (PAGE_SIZE - 1)) &
               ~(PAGE_SIZE - 1));
  assert(
      !mprotect(packet_buffer, PAGE_SIZE, PROT_READ | PROT_WRITE | PROT_EXEC));
  if (config.nx_support) {
    ...
    assert(!mprotect(packet_buffer + PAGE_SIZE, PAGE_SIZE,
  ...
  */
  ...
  while (move_next_range()) {
    /* sync_fprintf(stderr, "job: %d // range: ", job); print_range(stderr,
     * &search_range); sync_fprintf(stderr, "\n");  sync_fflush(stderr,true); */
    while (move_next_instruction()) {
      /* sync_fprintf(stderr, "job: %d // mc: ", job); print_mc(stderr, 16);
       * sync_fprintf(stderr, "\n"); sync_fflush(stderr,true); */
      pretext();
      for (i = 1; i <= MAX_INSN_LENGTH; i++) {
        ...
        /* SI_KERNEL appears with in, out, hlt, various retf, movabs, mov cr,
         * etc */
